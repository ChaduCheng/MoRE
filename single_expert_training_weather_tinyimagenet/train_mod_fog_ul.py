...
def train_fog (train_loader, device,optimizer,model,CE_loss, t, light, lr_schedule, epoch_i):
    b =  0   
    ...
    for images, labels in tqdm(train_loader):
        ...
        for i in range(0,images.shape[0]):
               ...
               # print(images[i])
                images_fog = add_fog(images[i], t, light)
                #print(images_fog)
                images[i] = images_fog 
# add fog to images
...
def val(val_loader, device, model,  basic_model,\
    ...
    if (acc_fog + acc_nat)/2 > best_acc_snow:
            print('saving..')
            ...
            best_acc_snow = ( acc_fog + acc_nat )/2
...
def train(args):
    ...
    for i in range(args.epochs):
        ...
        if args.heavy == 1:
            ...
            light = 1.0
            brightness = 20             
        print('t', t,  'light:', light)
        import os
        dir = args.out_dir
        if not os.path.exists(dir):
            os.makedirs(dir)
        args.checkpoint_loc = '{}{}_{}_{}_{}.pt'.format(dir, args.dataset,'fog',t,light)
        print(args.checkpoint_loc)
        
        train_fog (train_loader, device,optimizer,model,CE_loss, t, light, lr_schedule, i)
