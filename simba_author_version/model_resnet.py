...
class MoE_ResNet18(nn.Module):
    ...
    def forward(self, x):
        ...
        #self.output_dim = self.output_dim.to(device)
        # print(x)
        # print(len(x))
        # print(x.shape)
        # print(weights)
        ...
        weights_aver = weights_aver.to(device)
        sum_final = sum(out_final)
        # out_final = torch.Tensor(sum_final)
        sum_final = sum_final.to(device)
        #out_final = out_final.to(device)

        return sum_final    ### out will have the shape [batch_size, output_dim]
