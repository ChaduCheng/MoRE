...
from utils2.core import *
from utils2.torch_backend import *
...
if params.dataset == 'cifar':
    ...
    output_classes = 200
transform = utils.get_transformation(params.dataset)
dataset = utils.get_dataset(params.dataset, transform, None)
train_loader = torch.utils.data.DataLoader(dataset['train_data'], batch_size = params.batch_size, shuffle=True)
test_loader = torch.utils.data.DataLoader(dataset['test_data'], batch_size = params.batch_size, shuffle=True)
train_batches, test_batches = train_loader, test_loader
...
# lr_schedule = lambda t: np.interp([t], [0, params.epochs*2//5, params.epochs*4//5, params.epochs], [0, 0.1, 0.005, 0])[0]
# endregion
...
model = ResNet18(output_classes).to(device)
for m in model.children(): 
    if not isinstance(m, nn.BatchNorm2d):
        m.half()   
if params.resume:
    # model.load_state_dict(torch.load(params.checkpoint_loc)['net'])
    utils.load_model(params.checkpoint_loc, model) 
...
for epoch_i in (range(1,epochs+1)):
    ...
    if choice == 6:
        ...
        train_loss, train_acc = epoch_adversarial(train_batches, lr_schedule, model, epoch_i, attack, criterion, opt = opt, device = device, epsilon_l_2 = 1.0, alpha_l_inf = 0.005)
    else:
        train_loss, train_acc = epoch_adversarial(train_batches, lr_schedule, model, epoch_i, attack, criterion, opt = opt, device = device)
    
    total_loss, total_acc   = epoch(test_batches, lr_schedule, model, epoch_i, criterion, opt = None, device = "cuda:0")
    ...
    # myprint('Epoch: {6}, Clean Acc: {5:.4f} Train Acc: {4:.4f}, Test Acc 2: {3:.4f}, Test Acc inf: {2:.4f}, Time: {1:.1f}, lr: {0:.4f}'.format(lr, time.time()-start_time, total_acc_3, total_acc_2, train_acc, total_acc, epoch_i))
    myprint('Epoch: {7}, Clean Acc: {6:.4f} Train Acc msd: {5:.4f}, Test Acc 1 l1: {4:.4f}, Test Acc 2 l2 : {3:.4f}, Test Acc linf: {2:.4f}, Time: {1:.1f}, lr: {0:.4f}'.format(lr, time.time()-start_time, total_acc_3, total_acc_2,total_acc_1,train_acc, total_acc, epoch_i))
    # print('clean acc value is:', total_acc, 'l1 acc is:', total_acc_1)
    if epoch_i %5 == 0:
