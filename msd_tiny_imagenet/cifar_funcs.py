...
def pgd_l2(model, X, y, epsilon=1.0, alpha=2.5 * 1.0 / 7, num_iter = 7, device = "cuda:0", restarts = 0, version = 0):
...
def pgd_l1_topk(model, X,y, epsilon = 12.0, alpha = 2.5 * 12.0 / 7, num_iter = 7, k = int(np.round((1 - 0.95) * 3*64*64)), device = "cuda:0", restarts = 0, version = 0):
# def pgd_l1_topk(model, X, y, epsilon=12, alpha=1.5, num_iter=20, k=20, device="cuda:0", restarts=1, version=0):
...
def pgd_linf(model, X, y, epsilon=8.0/255, alpha=0.01, num_iter = 7, device = "cuda:0", restarts = 0, version = 0):
    ...
    for i in range (restarts):
        ...
        for t in range(num_iter):
            output = model(X+delta)
            incorrect = output.max(1)[1] != y 
            correct = (1-incorrect).unsqueeze(1).unsqueeze(1).unsqueeze(1).half()
            correct = 1.0 if version == 0 else correct
...
def pgd_l0(model, X,y, epsilon = 12, alpha = 1, num_iter = 0, device = "cuda:0"):
    delta = torch.zeros_like(X, requires_grad = True)
...
def msd_v0(model, X,y, epsilon_l_inf = 8.0/255, epsilon_l_2= 1.0, epsilon_l_1 = 12.0,
                alpha_l_inf = 0.01, alpha_l_2 = 2.5 * 1.0 / 7, alpha_l_1 = 2.5 * 12.0 / 7, num_iter = 7, device = "cuda:0"):
    delta = torch.zeros_like(X,requires_grad = True)
    max_delta = torch.zeros_like(X)
    ...
    for t in range(num_iter):
        ...
        with torch.no_grad():                
            ...
            #For L1
            k = int(np.round((1 - 0.95) * 3*64*64))
            alpha_l_1 = 2.5 * 12.0 / 7
            ...
            # print("delta_l_1, delta_l_2, delta_l_inf", delta_l_1, delta_l_2, delta_l_inf)
...
def epoch(loader, lr_schedule,  model, epoch_i, criterion = nn.CrossEntropyLoss(), opt=None, device = "cuda:0", stop = False):
    ...
    for i,batch in enumerate(loader): 
        X,y = batch
        X, y = X.half().to(device), y.to(device)
        output = model(X)
...
def epoch_adversarial_saver(batch_size,loader, model, attack, epsilon, num_iter, device = "cuda:0", restarts = 0):
