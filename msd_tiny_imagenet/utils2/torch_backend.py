...
import torchvision
from utils2.core import build_graph, cat, to_numpy
...
def warmup_cudnn(model, batch_size):
    ...
    batch = {
        'input': torch.Tensor(np.random.rand(batch_size,3,64,64)).cuda().half(), 
...
class Batches():
    def __init__(self, dataset, batch_size, shuffle, set_random_choices=False, num_workers=0, drop_last=False, gpu_id = 1):
        self.dataset = dataset
        self.batch_size = batch_size
        self.set_random_choices = set_random_choices
        torch.cuda.set_device(gpu_id)
        self.device = torch.device("cuda:{0}".format(str(gpu_id)) if torch.cuda.is_available() else "cpu")
    ...
    def __iter__(self):
        ...
        return ({'input': x.to(self.device).half(), 'target': y.to(self.device).long()} for (x,y) in self.dataloader)
