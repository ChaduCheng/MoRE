...
def test_foolbox(model_name, max_tests):
    ...
    for j in range(len(attacks_list)):
        ...
        for i,batch in enumerate(test_batches): 
            ...
            X,y = batch
            X, y = X.half().to(device).float(), y.to(device).float()
            image = X[0,:,:,:].view(3,64,64).detach().cpu().numpy().astype('float32')
            label = y[0].long().item()
            for r in range (restarts):
                ...
                try :
                    adversarial.all()
                    adv = torch.from_numpy(adversarial).float().view(1,3,64,64).to(device)
...
def fast_adversarial_DDN(model_name):
    ...
    for i,batch in enumerate(test_batches): 
        x,y = batch
        x, y = x.half().to(device).float(), y.to(device)
        restarts = 1        
