...
def ensemble_train(loss_l2, loss_linf, loss_fog, loss_snow, size_train, b, lr_schedule, optimizer, epoch_i):

    all_loss = []

    # l_clean_max = max(loss_clean)
    # all_loss.append(l_clean_max)
    l_l2_max = max(loss_l2)
    all_loss.append(l_l2_max)
    l_linf_max = max(loss_linf)
    all_loss.append(l_linf_max)
    l_fog_max = max(loss_fog)
    all_loss.append(l_fog_max)
    l_snow_max = max(loss_snow)
...
def train(args):
    ...
    for i in range(args.epochs):
        ...
        for images, labels in tqdm(train_loader):
            ...
            ensemble_train(loss_l2, loss_linf, loss_fog, loss_snow, size_train, b, lr_schedule, optimizer, i)
        ...
        acc_nat_1, acc_l2, acc_linf  = val_adv(test_loader, device, model, model, AttackPGD, config_l2, config_linf, attack, \
                                                                                        correct_final_nat_1, correct_final_l2,  correct_final_linf,  args.checkpoint_loc)

        acc_nat_2, acc_fog, acc_snow = val_nat(test_loader, device, model, correct_final_nat_2, correct_final_fog, correct_final_snow, args.checkpoint_loc, config_fog, config_snow)
        ...
        # if (acc_nat_1 + sum(acc_l2) + sum(acc_linf) ) / 5 > best_acc_aver:
        #     print('saving..')
        #
        #     state = {
        #         'net': model.state_dict(),
        ...
        #     best_acc_l2 = acc_l2
        #     best_acc_linf = acc_linf
        #     best_acc_aver = (best_acc_nat_1 + sum(best_acc_l2) + sum(best_acc_linf)) / 5
