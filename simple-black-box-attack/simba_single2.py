...
class adv_cifar(datasets.CIFAR10):
  ...
  def __getitem__(self, index):
    ...
    img = simba_single(model=self.model, x=img.unsqueeze(dim=0), y=target, num_iters=10, epsilon=4./255).squeeze(dim=0)
    return img, target
trainset2 =adv_cifar(model=net, root='./data', train=True, download=True, transform=utils.CIFAR_TRANSFORM)
trainloader2 = torch.utils.data.DataLoader(
    trainset2, batch_size=256, shuffle=True, num_workers=2)
testset2 = adv_cifar(model=net,
    root='./data', train=False, download=True, transform=utils.CIFAR_TRANSFORM)
testloader2 = torch.utils.data.DataLoader(
    testset2, batch_size=256, shuffle=False, num_workers=2)
# test(net,testloader)
...
#                 inputs[index] = simba_single(model=net, x=img.unsqueeze(0), y=tar, num_iters=10, epsilon=4./255).squeeze(dim=0)
#             outputs = net(inputs)
#             loss = criterion(outputs, targets)
