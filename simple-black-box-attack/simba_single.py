...
parser.add_argument('--stop_image_at', '-st',type=int,default=None)
...
def simba_single(model, x, y, num_iters=10000, epsilon=0.2):
    ...
    for i in tqdm(range(num_iters)):
        ...
        tmp = project(new_input = (x - diff.view(x.size()).to(device)), orig_input=x, eps=epsilon,norm=args.norm)
        left_prob = get_probs(model, tmp, y)
        if left_prob < last_prob:
            ...
            tmp =  project(new_input = (x + diff.view(x.size()).to(device)), orig_input=x, eps=epsilon,norm=args.norm)
            right_prob = get_probs(model, tmp, y)
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate((testloader)):
            ...
            for index, (img, tar) in enumerate(zip(inputs, targets)):
                list[index] = simba_single(model=net, x=img.unsqueeze(0), y=tar, num_iters=args.iteration, epsilon=float(args.epsilon)*1.0/255)
