...
parser.add_argument('--epsilon', type=float, default=1.0, help='step size per iteration')
...
if args.model_select ==0:
    ...
    args.model_path = './checkpoint/l2_0.5.pt'
elif args.model_select ==2:
    args.model_path ='./checkpoint/l2_1.0.pt'
else:
...
name = "l2_attack_"+ "m:"+str(args.model_select)+ "_v:"+str(args.epsilon)+ "_p:"+ str(args.pixel_attack)+"_"
def write_str_to_file(string, text_file):
    text_file = open(text_file, "a+")
    text_file.write(str(string) + "\n")
    text_file.close()
def save_print(content,directory='./logs_l2_5/',path = name+datetime.now().strftime("%m%d%Y_%H%M%S.txt")):
...
def dct_attack_batch(model, images_batch, labels_batch, max_iters, freq_dims, stride, epsilon, order='rand', targeted=False, pixel_attack=False, log_every=1):
    ...
    for k in range(max_iters):
        ...
        adv = project(adv,orig_inputs[remaining_indices], eps=epsilon,norm='l2')
