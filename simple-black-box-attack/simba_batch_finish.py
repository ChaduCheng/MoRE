...
if args.iteration == -1:
    args.iteration = int(3*32*32*args.batch_size*args.multiple)
if 0:
...
def test2(net,testloader):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate((testloader)):
            ...
            inputs2 = simba_single(model=net, x=inputs, y=targets, num_iters=args.iteration, epsilon=float(args.epsilon)*1.0/255)
