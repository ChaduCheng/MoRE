...
def testing(val_loader, device, model, basic_model, AttackPGD, config_l1, config_l2, config_linf, attack, \
        correct_final_nat, best_acc_nat, correct_final_l2, best_acc_l2, correct_final_linf, \
        best_acc_linf, best_acc_aver, checkpoint_loc):
    acc_linf = []
    acc_l2 = []
    ...
    for i in config_l1:
        j = j + 1
        # print('Valuation l_2  epsilon: ' + str(config_l2[i]['epsilon']))
        print('Valuation l_1  epsilon: ' + str(config_l1[i]['epsilon']))
        ...
        for images_2, labels in tqdm(val_loader):
            ...
            pred = prediction.argmax(dim=1, keepdim=True)
            correct_l1 = pred.eq(labels.view_as(pred)).sum().item()
            # correct_final.append(correct)
            correct_final_l1 = correct_final_l1 + correct_l1
    ...
    if (acc_nat * 2 +sum(acc_l1) + sum(acc_l2) + sum(acc_linf)) / 6 > best_acc_aver:
        print('updating..')
        best_acc_nat = acc_nat
        best_acc_l1 = acc_l1
        best_acc_l2 = acc_l2
        best_acc_linf = acc_linf
        best_acc_aver = (best_acc_nat * 2 +sum(best_acc_l1)+ sum(best_acc_l2) + sum(best_acc_linf)) / 6
...
def test(args):
    ...
    config_l1_2 = {
        'epsilon': eps3,
        'num_steps': 7,
        'step_size': 2.5 * eps3 / 7,
        'random_start': True,
        'loss_func': 'xent',
        '_type': 'l1'
    }
    config_l1 = dict(config_l1_0=config_l1_0, config_l1_1 = config_l1_1, config_l1_2= config_l1_2)
    attack = 'true'
...
#     model = MoE_ResNet18_adv(args.num_experts, output_classes)
    ...
    for i in range(args.epochs):
        ...
        acc_nat, best_acc_nat, acc_l1, best_acc_l1, acc_l2, best_acc_l2, acc_linf, best_acc_linf, best_acc_aver = testing(test_loader, device,
                                                                                                 model, model,
                                                                                                 AttackPGD, config_l1, config_l2,
                                                                                                 config_linf, attack, \
                                                                                                 correct_final_nat,
                                                                                                 best_acc_nat,
                                                                                                 correct_final_l2,
